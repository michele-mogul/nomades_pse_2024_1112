...
from helpers.csv_wrapper import read_users, write_user
...
def login():
    ...
    if uid_ and pwd_:
        if any(user["uid"] == uid_ and user["pwd"] == hash_pwd(pwd_ + user['salt']) for user in read_users(CSV_FILE)):
            persist_login(uid_)
...
def register():
    ...
    pwd_ = request.form.get("tbx_pwd")
    if uid_ and pwd_:
        if any(user["uid"] == uid_ for user in read_users(CSV_FILE)):
            flash("User already exists", "warning")
            return redirect("/register")

        salt = os.urandom(32).hex()
        write_user(CSV_FILE, uid_, hash_pwd(pwd_ + salt), salt)
        persist_login(uid_)
        flash("Success", "success")
        return redirect('/private')
    flash("Invalid Input", "warning")
